...
In this take-home exercise, you are tasked to reveal the spatia-temporal patterns of COVID-19 case at DKI Jakarta.  The temporal interval must be at *epidemiology week* (epi week) and the geographic should be at *sub-district* or [*kelurahan*](https://en.wikipedia.org/wiki/Subdivisions_of_Indonesia) in Indonesia language. The specific tasks are as follow:

### Data Extraction, Wrangling and Integration

- Download the daily COVID-19 data of DKI Jakarta, 
- Extract ID_KEL,	Nama_provinsi, nama_kota,	nama_kecamatan	nama_kelurahan, POSITF and meninggal from ```data``` worksheet of the daily COVID-19 data,
- Integrate the daily data into a data frame by month,
- Download the DKI Jarkata shapefile,
- Import the shapefile into R and save it in simple feature data frame format,
- Exclude all the outer islands from the DKI Jakarta sf data frame.
- Include the first nine fields in the DKI Jakarta sf data frame.
